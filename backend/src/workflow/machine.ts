...
export const registrationTransitions: TransitionDefinition[] = [
  // ===== DRAFT PHASE =====
  {
...
export const workflowOpenQuestions: WorkflowOpenQuestion[] = [
  {
    id: 'Q1_BRAND_SKIP_SUPERVISOR',
    title: 'BRAND creator workflow skip (Q1 - CONFIRMED)',
    interpretationA:
      'If creator has BRAND role, registration skips Supervisor Review and goes directly to Brand Manager Approval.',
  ...
  },
  {
    ...
    interpretationA:
      'Approver must be direct manager of creator via manager_id field (organizational hierarchy).',
...
export function evaluateRegistrationTransition(
  ...
  const candidate = registrationTransitions.find(
    (transition) =>
      transition.action === action && transition.from.includes(context.currentState),
  );

  if (!candidate) {
    const actionExistsForState = registrationTransitions.some((transition) =>
      transition.from.includes(context.currentState),
    );
