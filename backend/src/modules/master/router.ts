...
masterRouter.post('/channels', requirePermission('channel.create'), async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    const result = await db.query(
      `INSERT INTO channels(code, name, description, location_id, created_by, updated_by)
       VALUES ($1, $2, $3, $4, $5, $5)
       RETURNING *`,
      [code, name.trim(), description ?? null, location_id, req.user?.id],
    );
...
masterRouter.patch('/channels/:id', requirePermission('channel.update'), async (req: AuthenticatedRequest, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    const result = await db.query(
      `UPDATE channels
       ...
       WHERE id = $1
       RETURNING *`,
      [id, name?.trim(), description ?? null, status, location_id, req.user?.id],
    );
...
masterRouter.delete('/channels/:id', requirePermission('channel.delete'), async (req: AuthenticatedRequest, res) => {
  try {
...
masterRouter.post('/categories', requirePermission('category.create'), async (req: AuthenticatedRequest, res) => {
  ...
  try {
    const result = await db.query(
      `INSERT INTO categories(code, name, description, unit_price, status, created_by, updated_by)
       VALUES (UPPER($1), $2, $3, $4, 'ACTIVE', $5, $5)
       RETURNING *`,
      [code, name.trim(), description ?? null, unit_price ?? 0, req.user?.id],
    );
...
masterRouter.patch('/categories/:id', requirePermission('category.update'), async (req: AuthenticatedRequest, res) => {
  const { id } = req.params;
  ...
  try {
    const result = await db.query(
      `UPDATE categories
       ...
       WHERE id = $1
       RETURNING *`,
      [id, name?.trim(), description ?? null, unit_price, status, req.user?.id],
    );
...
masterRouter.delete('/categories/:id', requirePermission('category.delete'), async (req: AuthenticatedRequest, res) => {
  try {
...
masterRouter.post('/locations', requirePermission('location.create'), async (req: AuthenticatedRequest, res) => {
  ...
  try {
    const result = await db.query(
      `INSERT INTO locations(code, name, address_line, latitude, longitude, status, created_by, updated_by)
       VALUES (UPPER($1), $2, $3, $4, $5, 'ACTIVE', $6, $6)
       RETURNING *`,
      [code, name.trim(), address_line ?? null, latitude ?? null, longitude ?? null, req.user?.id],
    );
...
masterRouter.patch('/locations/:id', requirePermission('location.update'), async (req: AuthenticatedRequest, res) => {
  const { id } = req.params;
  ...
  try {
    const result = await db.query(
      `UPDATE locations
       ...
       WHERE id = $1
       RETURNING *`,
      [id, name?.trim(), address_line ?? null, latitude ?? null, longitude ?? null, status, req.user?.id],
    );
...
masterRouter.delete('/locations/:id', requirePermission('location.delete'), async (req: AuthenticatedRequest, res) => {
  try {
...
masterRouter.post('/contents', requirePermission('content.create'), async (req: AuthenticatedRequest, res) => {
  ...
  try {
    const created = await db.query(
      ...
      RETURNING *`,
      [channelId, categoryId ?? null, name.trim(), description ?? null, startDate, endDate, JSON.stringify(imageKeys), req.user?.id],
    );
...
masterRouter.patch('/contents/:id', requirePermission('content.update'), async (req: AuthenticatedRequest, res) => {
  const { id } = req.params;
  ...
  try {
    const result = await db.query(
      `UPDATE advertising_contents
       ...
       WHERE id = $1
       RETURNING *`,
      [id, name?.trim(), description ?? null, startDate, endDate, imageKeys ? JSON.stringify(imageKeys) : null, req.user?.id],
    );
