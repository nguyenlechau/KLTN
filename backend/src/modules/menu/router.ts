...
menuRouter.post('/', async (req, res) => {
  try {
    ...
    const result = await db.query(
      `INSERT INTO menus (code, name, label, icon, order_position, parent_id, status, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW())
       RETURNING *`,
      [code, name, label || null, icon || null, order_position || 999, parent_id || null, status || 'ACTIVE']
    );
