...
export async function createUser(payload: CreateUserPayload) {
  ...
  const result = await db.query(
    ...
    [payload.email.toLowerCase(), payload.fullName, passwordHash, 'ACTIVE', payload.role]
  );
...
export async function getUserById(id: string) {
  const result = await db.query(
    `SELECT u.id, u.email, u.full_name, r.code as role, u.status, u.created_at, u.updated_at
     FROM users u
...
export async function authenticateUser(email: string, password: string) {
  ...
  const result = await db.query(
    `SELECT u.id, u.email, u.full_name, r.code as role, u.status, u.created_at
     FROM users u
     JOIN roles r ON u.role_id = r.id
     WHERE lower(u.email) = lower($1) AND u.password_hash = $2 AND u.deleted_at IS NULL`,
    [email, passwordHash]
  );
