...
export class ApprovalRoutingService {
  ...
  async getNextApprover(
    ...
    try {
      const creator = await this.db.query(
        'SELECT id, role, manager_id, department_id FROM users WHERE id = $1',
        [creatorId]
      );
   ...
   */
  private async routeToSupervisor(creatorData: any): Promise<ApprovalDecision | null> {
    if (!creatorData.manager_id) {
      throw new Error(
        `Cannot route to supervisor: creator has no manager_id set. User must have organizational hierarchy configured.`
      );
    }
    ...
    try {
      const manager = await this.db.query(
        `SELECT id, full_name, role, department_id 
         FROM users 
         WHERE id = $1 AND role IN ('APPROVER', 'APPROVER_HO')`,
        [creatorData.manager_id]
      );

      if (manager.rows.length === 0) {
        throw new Error(
          `Manager of creator is not an APPROVER or APPROVER_HO. Cannot proceed with approval.`
        );
      }
   ...
   */
  private async routeToBrand(registrationId: string): Promise<ApprovalDecision | null> {
    try {
      // For MVP: assign to first available BRAND user
      // Phase 2: Use category-based assignment configuration
      const brand = await this.db.query(
        `SELECT id, full_name, role, department_id 
         FROM users 
         WHERE role = 'BRAND'
         LIMIT 1`
   ...
   * Route to BRAND_MANAGER
   */
  private async routeToBrandManager(creatorData: any): Promise<ApprovalDecision | null> {
    try {
      // Get direct manager of creator if they're BRAND, otherwise get any BRAND_MANAGER
      let managerQuery: string;
      let params: any[];
   ...
   */
  async validateApproverAuthorization(
    approverId: string,
    registrationId: string,
    currentStatus: string
  ): Promise<boolean> {
   ...
   */
  async getApprovalChain(creatorId: string): Promise<any[]> {
    try {
      const creator = await this.db.query(
      ...
      while (currentUserId) {
        const user = await this.db.query(
          'SELECT id, full_name, role, manager_id FROM users WHERE id = $1',
          [currentUserId]
        );
   ...
   */
  async isManagerOf(managerId: string, userId: string): Promise<boolean> {
    try {
