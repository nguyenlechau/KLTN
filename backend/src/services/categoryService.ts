...
export async function createCategory(
  ...
  const existing = await queryOne(
    `SELECT id FROM categories WHERE code = $1 OR name = $2`,
    [data.code, data.name]
  );
  if (existing) throw new Error('Category code or name already exists');

  await query(
    `INSERT INTO categories (id, code, name, format, description, unit_price, unit_of_measure, status, created_by, created_at, updated_at)
...
export async function getCategoryByCode(code: string): Promise<Category | null> {
  return queryOne<Category>(
    `SELECT * FROM categories WHERE code = $1 AND deleted_at IS NULL`,
    [code]
  );
}
...
export async function listCategories(
  ...
  const items = await queryAll<Category>(
    `SELECT * FROM categories ${whereClause} ORDER BY code ASC 
...
export async function updateCategory(
  ...
  for (const field of editableFields) {
    if (field in data && data[field as keyof Category] !== undefined) {
  ...
  if (updateFields.length === 0) return await getCategoryById(id) as Category;
  ...
  await query(
    `UPDATE categories SET ${updateFields.join(', ')} WHERE id = $${paramCount} AND deleted_at IS NULL`,
    values
  );
 ...
 */
export async function canDeactivateCategory(categoryId: string): Promise<{ can: boolean; itemCount: number }> {
  const result = await queryOne<{ count: string }>(
    `SELECT COUNT(*) as count FROM physical_items 
     WHERE category_id = $1 AND status = 'TREO' AND deleted_at IS NULL`,
    [categoryId]
  );
 ...
 */
export async function deactivateCategoryItems(categoryId: string): Promise<void> {
  const now = new Date().toISOString();
  await query(
    `UPDATE physical_items SET status = 'INACTIVE', updated_at = $1 
     WHERE category_id = $2 AND status = 'ACTIVE' AND deleted_at IS NULL`,
    [now, categoryId]
  );
}
...
export async function updateCategoryPrice(
  categoryId: string,
  newPrice: number
): Promise<{ updated: Category; affectedRegistrations: number }> {
  ...
  const now = new Date().toISOString();
  await query(
    `UPDATE categories SET unit_price = $1, updated_at = $2 WHERE id = $3`,
    [newPrice, now, categoryId]
  );

  // Recalculate totals for registrations in approval phase (not past manager approval)
  const affectedResult = await queryOne<{ count: string }>(
    `SELECT COUNT(DISTINCT ri.registration_id) as count FROM registration_items ri
     ...
     AND r.prices_locked_at IS NULL`,
    [categoryId]
  );
  ...
  const updated = await getCategoryById(categoryId) as Category;
  return {
    updated,
