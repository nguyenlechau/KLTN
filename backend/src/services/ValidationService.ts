...
export class ValidationService {
  ...
  async validateChannelCode(code: string): Promise<ValidationError | null> {
  ...
  async validateChannelDeactivation(channelId: string): Promise<ValidationError | null> {
    try {
      const result = await this.db.query(
        'SELECT COUNT(*) as count FROM locations WHERE channel_id = $1 AND status = $2 AND deleted_at IS NULL',
        [channelId, 'ACTIVE']
      );
  ...
  async cascadeChannelDeactivation(channelId: string): Promise<void> {
    try {
      await this.db.query(
        'UPDATE locations SET status = $1, updated_at = NOW() WHERE channel_id = $2 AND deleted_at IS NULL',
        ['INACTIVE', channelId]
      );
  ...
  async validateCategoryName(name: string): Promise<ValidationError | null> {
  ...
  async validateCategoryPriceChange(categoryId: string, newPrice: number): Promise<ValidationError | null> {
  ...
  async validateCategoryDeactivation(categoryId: string): Promise<ValidationError | null> {
    try {
      const result = await this.db.query(
        'SELECT COUNT(*) as count FROM physical_items WHERE category_id = $1 AND status = $2 AND deleted_at IS NULL',
        [categoryId, 'TREO']
      );
  ...
  async cascadeCategoryDeactivation(categoryId: string): Promise<void> {
    try {
      await this.db.query(
        'UPDATE physical_items SET status = $1, updated_at = NOW() WHERE category_id = $2 AND deleted_at IS NULL',
        ['INACTIVE', categoryId]
      );
    } catch (error) {
  ...
  async validatePhoneNumber(phone?: string): Promise<ValidationError | null> {
  ...
  async validateEmail(email?: string): Promise<ValidationError | null> {
  ...
  async validateLocationDeactivation(locationId: string): Promise<ValidationError | null> {
    try {
      const result = await this.db.query(
        'SELECT COUNT(*) as count FROM physical_items WHERE location_id = $1 AND status = $2 AND deleted_at IS NULL',
        [locationId, 'TREO']
      );
  ...
  async cascadeLocationDeactivation(locationId: string): Promise<void> {
    try {
      await this.db.query(
        'UPDATE physical_items SET status = $1, updated_at = NOW() WHERE location_id = $2 AND deleted_at IS NULL',
        ['INACTIVE', locationId]
      );
    } catch (error) {
  ...
  async validateItemDimensions(width?: number, length?: number): Promise<ValidationError | null> {
    if (width !== undefined) {
  ...
  async validateItemEditability(itemId: string): Promise<ValidationError | null> {
    try {
      const result = await this.db.query(
  ...
  async propagateItemStatusFromParent(itemId: string): Promise<void> {
    try {
      // Get item's category and position status
      const item = await this.db.query(
        'SELECT category_id, location_id FROM physical_items WHERE id = $1',
        [itemId]
      );
  ...
  async validateContentDates(startDate: Date, endDate: Date): Promise<ValidationError | null> {
  ...
  async computeContentStatus(endDate: Date): Promise<'Còn hạn' | 'Hết hạn'> {
  ...
  async validateRegistrationSubmit(registrationId: string): Promise<ValidationError | null> {
    try {
      const result = await this.db.query(`
        SELECT COUNT(*) as count 
  ...
  async validateApproverIsDirectManager(
    approverId: string,
    creatorId: string
  ): Promise<ValidationError | null> {
    try {
  ...
  async validateAcceptanceCompletion(registrationId: string): Promise<ValidationError[]> {
  ...
  async validateRegistrationBudget(registrationId: string): Promise<ValidationError | null> {
    try {
      const result = await this.db.query(`
        SELECT 
          r.budget_estimate,
  ...
  async getAccessibleRegistrations(userId: string, userRole: string): Promise<string[]> {
    try {
      let query = '';
