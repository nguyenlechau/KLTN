...
type DbClient = Pick<Pool, 'query' | 'connect'>;

async function getFirstId(db: DbClient, sql: string) {
  const result = await db.query(sql);
  return result.rowCount ? result.rows[0].id as string : null;
}

export async function seedSampleCampaignData(db: DbClient) {
  ...
  const contentIds = (await db.query('SELECT id FROM advertising_contents ORDER BY created_at ASC LIMIT 2')).rows.map((row) => row.id as string);
  ...
  for (const seed of itemSeeds) {
    ...
    const inserted = await db.query(
      ...
      RETURNING id`,
      [channelId, categoryId, locationId, seed.seqNo, seed.itemCode, seed.itemName, null, userId],
    );
  ...
  for (let index = 0; index < registrations.length; index += 1) {
    ...
    await db.query(
      `INSERT INTO registration_items(registration_id, physical_item_id, unit_price, quantity)
       VALUES ($1, $2, $3, 1)`,
      [registrationId, itemIds[index], unitPrice + index * 1500],
    );
  }
