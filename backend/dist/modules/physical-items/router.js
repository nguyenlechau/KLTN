...
physicalItemsRouter.get('/', requirePermission('physical_item.view'), async (_req, res) => {
    const result = await db.query('SELECT * FROM physical_items ORDER BY created_at DESC');
    res.json(result.rows);
});
physicalItemsRouter.post('/wizard/preview', requirePermission('physical_item.create'), async (req, res) => {
    ...
    const [category, location, seq] = await Promise.all([
        ...
        db.query('SELECT COALESCE(MAX(seq_no), 0)::int AS max_seq FROM physical_items WHERE category_id = $1 AND location_id = $2', [categoryId, locationId]),
    ]);
    ...
    const generatedItems = Array.from({ length: quantity }).map((_, index) => {
        ...
        return {
            ...
            inferredStatus: categoryData.status === 'ACTIVE' && locationData.status === 'ACTIVE'
                ? 'ACTIVE'
                : 'INACTIVE',
        };
    });
...
physicalItemsRouter.post('/wizard/commit', requirePermission('physical_item.create'), async (req, res) => {
    const { categoryId, locationId, items } = req.body;
    ...
    const statusCheck = await db.query(`SELECT c.status AS category_status, l.status AS location_status,
      ...
      WHERE c.id = $1`, [categoryId, locationId]);
    if (!statusCheck.rowCount) {
        res.status(404).json({ message: 'Category/location not found' });
        return;
    }
    const inheritedStatus = statusCheck.rows[0].category_status === 'ACTIVE' &&
    ...
    try {
        ...
        const created = [];
        for (const item of items) {
            const result = await client.query(`INSERT INTO physical_items(
...
physicalItemsRouter.patch('/:id', requirePermission('physical_item.update'), async (req, res) => {
    const { id } = req.params;
    ...
    const updated = await db.query(`UPDATE physical_items
        SET width = COALESCE($2, width),
            ...
            updated_at = NOW()
      WHERE id = $1
      RETURNING *`, [id, width, length, imageKey, description, req.user?.id]);
