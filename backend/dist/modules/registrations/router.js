...
import { isBudgetWithinLimit, isEndDateAfterStartDate, isNonWhitespaceText, isValidPhone10Digits, } from '../../utils/validation.js';
...
registrationsRouter.post('/', requirePermission('registration.create'), async (req, res) => {
    const { campaignName, campaignDescription, budgetEstimate, startDate, endDate, documentKey, representativeName, representativePhone, contentMode, contentId, newContent, selectedItemIds, } = req.body;
    if (!isNonWhitespaceText(campaignName) ||
    ...
    try {
        ...
        if (contentMode === 'NEW') {
            ...
            const contentInsert = await client.query(`INSERT INTO advertising_contents(
        ...
        if (contentMode === 'EXISTING') {
            const valid = await client.query(`SELECT id FROM advertising_contents
          WHERE id = $1
            AND CURRENT_DATE <= end_date`, [contentId]);
            if (!valid.rowCount) {
                await client.query('ROLLBACK');
        ...
        const registrationInsert = await client.query(`INSERT INTO advertising_registrations(
      ...
      RETURNING *`, [
        ...
        if (Array.isArray(selectedItemIds) && selectedItemIds.length > 0) {
            const itemPrices = await client.query(`SELECT pi.id AS physical_item_id, c.unit_price
           FROM physical_items pi
           JOIN categories c ON c.id = pi.category_id
          WHERE pi.id = ANY($1::uuid[])`, [selectedItemIds]);
            for (const row of itemPrices.rows) {
                await client.query(`INSERT INTO registration_items(registration_id, physical_item_id, unit_price, quantity)
           VALUES ($1, $2, $3, 1)`, [registration.id, row.physical_item_id, row.unit_price]);
        ...
        await client.query(`INSERT INTO audit_logs(entity_type, entity_id, user_id, action, old_value, new_value)
       VALUES ('advertising_registrations', $1, $2, 'CREATE', NULL, $3::jsonb)`, [registration.id, req.user?.id, JSON.stringify(registration)]);
...
registrationsRouter.post('/:id/workflow-action', requirePermission('registration.view'), async (req, res) => {
    const { id } = req.params;
    ...
    const itemResult = await db.query(`SELECT pi.id,
            pi.status,
      ...
      WHERE ri.registration_id = $1`, [id]);
    const workflowItems = itemResult.rows.map((row) => ({
        id: row.id,
        status: row.status,
        deploymentImageUploaded: row.deployment_image_uploaded,
        inactiveNote: row.inactive_note,
    }));
    const totalResult = await db.query('SELECT COALESCE(SUM(total_amount),0)::numeric AS total FROM registration_items WHERE registration_id = $1', [id]);
    ...
    try {
        await client.query('BEGIN');
        const updated = await client.query(`UPDATE advertising_registrations
          SET status = $2,
              approved_at = CASE WHEN $3::text = 'SET_APPROVED_AT' THEN NOW() ELSE approved_at END,
              updated_by = $4,
              updated_at = NOW()
        WHERE id = $1
        RETURNING *`, [id, evaluation.nextState, evaluation.transition.sideEffects.includes('SET_APPROVED_AT') ? 'SET_APPROVED_AT' : null, req.user?.id]);
        await client.query(`INSERT INTO audit_logs(entity_type, entity_id, user_id, action, old_value, new_value)
