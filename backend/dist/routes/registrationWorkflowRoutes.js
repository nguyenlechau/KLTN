 ...
 */
router.post('/:id/submit', validateToken, requireRole(['INPUTTER', 'INPUTTER_HO', 'BRAND']), async (req, res) => {
    try {
        ...
        // Get registration
        const regResult = await services.db.query('SELECT * FROM ad_registrations WHERE id = $1', [registrationId]);
        if (regResult.rows.length === 0) {
            return res.status(404).json({ error: 'Registration not found' });
        }
        const registration = regResult.rows[0];
        // Get user role
        const userResult = await services.db.query('SELECT role FROM users WHERE id = $1', [userId]);
        ...
        if (userRole === 'BRAND') {
            ...
            const approvalDecision = await services.approval.getNextApprover(registrationId, registration.status, userId);
            if (!approvalDecision) {
        ...
        // Update registration
        await services.db.query('UPDATE ad_registrations SET status = $1, current_approver_id = $2, updated_at = NOW() WHERE id = $3', [nextStatus, nextApproverId, registrationId]);
        // Log audit trail
        await services.audit.logTransition(userId, 'Registration', registrationId, registration.registration_no, registration.status, nextStatus, notes || `Submitted to ${nextStatus}`);
        res.json({
 ...
 */
router.post('/:id/approve', validateToken, requireRole(['APPROVER', 'APPROVER_HO', 'BRAND', 'BRAND_MANAGER']), async (req, res) => {
    try {
        const { id: registrationId } = req.params;
        ...
        const registration = regResult.rows[0];
        // Get approver info
        const approverResult = await services.db.query('SELECT role FROM users WHERE id = $1', [approverId]);
        ...
        switch (registration.status) {
            case 'CBQL Phê duyệt':
                ...
                // Assign to Brand
                const brandDecision = await services.approval.getNextApprover(registrationId, registration.status, registration.created_by);
            ...
            case 'P.Thương hiệu tiếp nhận': // Brand Intake
                ...
                const brandMgrDecision = await services.approval.getNextApprover(registrationId, registration.status, registration.created_by);
            ...
            case 'Trưởng phòng thương hiệu phê duyệt':
                ...
                nextStatus = 'Đã duyệt';
                // Lock pricing
                await services.db.query('UPDATE ad_registrations SET prices_locked = true, prices_locked_at = NOW() WHERE id = $1', [registrationId]);
                break;
        ...
        await services.db.query('UPDATE ad_registrations SET status = $1, current_approver_id = $2, updated_at = NOW() WHERE id = $3', [nextStatus, nextApproverId, registrationId]);
        // Log approval
        await services.audit.logApproval(approverId, 'Registration', registrationId, registration.registration_no, registration.status, nextStatus, true, notes || `Approved to: ${nextStatus}`);
        res.json({
 ...
 * Roles: APPROVER, APPROVER_HO, BRAND, BRAND_MANAGER
 */
router.post('/:id/request-revision', validateToken, requireRole(['APPROVER', 'APPROVER_HO', 'BRAND', 'BRAND_MANAGER']), async (req, res) => {
    try {
        const { id: registrationId } = req.params;
        ...
        await services.db.query('UPDATE ad_registrations SET status = $1, current_approver_id = $2, updated_at = NOW() WHERE id = $3', ['CBNV Điều Chỉnh', registration.created_by, registrationId]);
        // Log approval
        await services.audit.logApproval(reviewerId, 'Registration', registrationId, registration.registration_no, registration.status, 'CBNV Điều Chỉnh', false, `Revision requested: ${notes}`);
        res.json({
 ...
 */
router.post('/:id/begin-acceptance', validateToken, requireRole(['BRAND']), async (req, res) => {
    try {
        const { id: registrationId } = req.params;
        const userId = req.user?.id;
        const services = getServices(req);
        ...
        await services.db.query('UPDATE ad_registrations SET status = $1, updated_at = NOW() WHERE id = $2', ['Nghiệm thu', registrationId]);
        // Log transition
        await services.audit.logTransition(userId, 'Registration', registrationId, registration.registration_no, registration.status, 'Nghiệm thu', 'Beginning acceptance phase - upload deployment images');
        res.json({
 ...
 */
router.post('/:id/submit-acceptance', validateToken, requireRole(['BRAND']), async (req, res) => {
    try {
        const { id: registrationId } = req.params;
        const userId = req.user?.id;
        const services = getServices(req);
        ...
        const acceptanceErrors = await services.validation.validateAcceptanceCompletion(registrationId);
        if (acceptanceErrors.length > 0) {
            return res.status(400).json({ errors: acceptanceErrors });
        }
        // Move to Acceptance Review
        // Assign to Brand Manager
        const brandMgrResult = await services.db.query('SELECT id FROM users WHERE role = $1 LIMIT 1', ['BRAND_MANAGER']);
        const nextApproverId = brandMgrResult.rows[0]?.id || null;
        await services.db.query('UPDATE ad_registrations SET status = $1, current_approver_id = $2, updated_at = NOW() WHERE id = $3', ['Trưởng phòng nghiệm thu', nextApproverId, registrationId]);
        // Log transition
        await services.audit.logTransition(userId, 'Registration', registrationId, registration.registration_no, registration.status, 'Trưởng phòng nghiệm thu', 'Submitted for acceptance review');
        res.json({
 ...
 * Role: BRAND
 */
router.post('/:id/cancel-request', validateToken, requireRole(['BRAND']), async (req, res) => {
    try {
        const { id: registrationId } = req.params;
        const userId = req.user?.id;
        ...
        await services.db.query('UPDATE ad_registrations SET status = $1, updated_at = NOW() WHERE id = $2', ['CANCELLED', registrationId]);
        // Log cancellation
 ...
 */
router.get('/:id/history', validateToken, async (req, res) => {
    try {
        const { id: registrationId } = req.params;
        const { limit = 100, offset = 0 } = req.query;
        const services = getServices(req);
        const history = await services.audit.getHistory('Registration', registrationId, parseInt(limit), parseInt(offset));
        res.json({ history });
    }
    catch (error) {
        res.status(500).json({ error: error.message });
