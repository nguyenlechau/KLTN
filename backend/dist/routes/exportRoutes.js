 ...
 * }
 */
router.post('/export', validateToken, requireRole(['BRAND_MANAGER', 'ADMIN']), // Only managers can export
async (req, res) => {
    try {
 ...
 */
router.post('/registrations', validateToken, requireRole(['BRAND_MANAGER', 'ADMIN']), async (req, res) => {
    try {
 ...
 */
router.post('/items', validateToken, requireRole(['BRAND_MANAGER', 'ADMIN']), async (req, res) => {
    try {
 ...
 */
router.post('/content', validateToken, requireRole(['BRAND_MANAGER', 'ADMIN']), async (req, res) => {
    try {
        const { format = 'excel' } = req.body;
        const db = req.app.get('db');
        const exportService = new ExportService(db);
 ...
 */
router.post('/locations', validateToken, requireRole(['BRAND_MANAGER', 'ADMIN']), async (req, res) => {
    try {
        const { format = 'excel' } = req.body;
        const db = req.app.get('db');
        const exportService = new ExportService(db);
 ...
 */
router.post('/categories', validateToken, requireRole(['BRAND_MANAGER', 'ADMIN']), async (req, res) => {
    try {
        const { format = 'excel' } = req.body;
        const db = req.app.get('db');
        const exportService = new ExportService(db);
 ...
 */
router.post('/orders', validateToken, requireRole(['BRAND_MANAGER', 'ADMIN']), async (req, res) => {
    try {
        const { format = 'excel' } = req.body;
        const db = req.app.get('db');
        const exportService = new ExportService(db);
 ...
 */
router.getContentType = function (format) {
    switch (format) {
        case 'excel':
