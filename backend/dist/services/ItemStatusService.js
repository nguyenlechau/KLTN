...
export class ItemStatusService {
     ...
     */
    async propagateParentStatusChange(entityType, entityId, newStatus) {
        const transitions = [];
        try {
            ...
            if (entityType === 'category') {
                ...
                const items = await this.db.query('SELECT id FROM physical_items WHERE category_id = $1 AND deleted_at IS NULL', [entityId]);
                for (const item of items.rows) {
                    const updated = await this.db.query('UPDATE physical_items SET status = $1, updated_at = NOW() WHERE id = $2 RETURNING status', ['INACTIVE', item.id]);
                    transitions.push({
                        itemId: item.id,
                        oldStatus: 'ACTIVE',
                        newStatus: 'INACTIVE',
                ...
                const items = await this.db.query('SELECT id FROM physical_items WHERE location_id = $1 AND deleted_at IS NULL', [entityId]);
                for (const item of items.rows) {
                    await this.db.query('UPDATE physical_items SET status = $1, updated_at = NOW() WHERE id = $2', ['INACTIVE', item.id]);
                    transitions.push({
                        itemId: item.id,
                        oldStatus: 'ACTIVE',
                        newStatus: 'INACTIVE',
     ...
     */
    async markItemsAsInProgress(itemIds) {
        try {
            for (const itemId of itemIds) {
                await this.db.query('UPDATE physical_items SET status = $1, updated_at = NOW() WHERE id = $2', ['TREO', itemId]);
            }
        }
        catch (error) {
     ...
     */
    async revertItemsFromInProgress(itemIds) {
        try {
            for (const itemId of itemIds) {
                // Check parent status before reverting
                const item = await this.db.query(`SELECT c.status as category_status, l.status as location_status
           ...
           WHERE pi.id = $1`, [itemId]);
                if (item.rows.length > 0) {
                    ...
                    const newStatus = category_status === 'ACTIVE' && location_status === 'ACTIVE' ? 'ACTIVE' : 'INACTIVE';
                    await this.db.query('UPDATE physical_items SET status = $1, updated_at = NOW() WHERE id = $2', [newStatus, itemId]);
                }
            }
        }
        catch (error) {
     ...
     */
    async getRegistrationItemStatusCounts(registrationId) {
        try {
        ...
        WHERE ri.registration_id = $1`, [registrationId]);
            return {
     ...
     */
    async hasInProgressItems(registrationId) {
        try {
            const result = await this.db.query(`SELECT COUNT(*) as count
         FROM ad_registration_items ri
         JOIN physical_items pi ON ri.physical_item_id = pi.id
         WHERE ri.registration_id = $1 AND pi.status = 'TREO'`, [registrationId]);
     ...
     */
    async getInProgressItems(registrationId) {
        try {
            const result = await this.db.query(`SELECT pi.id, pi.item_code, pi.item_name, pi.status
         FROM ad_registration_items ri
         JOIN physical_items pi ON ri.physical_item_id = pi.id
         WHERE ri.registration_id = $1 AND pi.status = 'TREO'
         ORDER BY pi.item_code`, [registrationId]);
            return result.rows;
        }
        catch (error) {
