 ...
 */
async function generateContentCode() {
    const result = await queryOne(`SELECT MAX(CAST(SUBSTRING(content_code FROM 3) AS INTEGER)) as max_num 
...
export async function createContent(data) {
    ...
    await query(`INSERT INTO advertising_content 
...
export async function getContentById(id) {
    const content = await queryOne(`SELECT * FROM advertising_content WHERE id = $1 AND deleted_at IS NULL`, [id]);
    if (content) {
        content.status = calculateStatus(content.end_date);
    }
    return content;
}
export async function listContent(limit = 25, offset = 0, search) {
    ...
    const countResult = await queryOne(`SELECT COUNT(*) as count FROM advertising_content ${whereClause}`, params);
    const items = await queryAll(`SELECT * FROM advertising_content ${whereClause} 
...
export async function updateContent(id, data) {
    ...
    paramCount++;
    values.push(id);
    await query(`UPDATE advertising_content SET ${updateFields.join(', ')} WHERE id = $${paramCount} AND deleted_at IS NULL`, values);
...
export async function deleteContent(id) {
    await query(`DELETE FROM advertising_content WHERE id = $1`, [id]);
}
export async function addContentImage(contentId, imageUrl, imageKey, sequence) {
    const id = uuidv4();
    await query(`INSERT INTO content_images (id, content_id, image_url, image_key, sequence)
     VALUES ($1, $2, $3, $4, $5)`, [id, contentId, imageUrl, imageKey, sequence]);
    return {
        id,
...
export async function getContentImages(contentId) {
    return queryAll(`SELECT * FROM content_images WHERE content_id = $1 ORDER BY sequence ASC`, [contentId]);
}
export async function deleteContentImage(imageId) {
    await query(`DELETE FROM content_images WHERE id = $1`, [imageId]);
}
export async function cloneContent(contentId, createdBy) {
