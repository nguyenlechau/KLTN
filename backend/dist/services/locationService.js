...
export async function createLocation(data) {
    ...
    const existing = await queryOne(`SELECT id FROM locations WHERE position_code = $1 AND channel_id = $2 AND deleted_at IS NULL`, [data.position_code, data.channel_id]);
    if (existing)
        throw new Error('Position code already exists in this channel');
    await query(`INSERT INTO locations 
...
export async function getLocationById(id) {
    return queryOne(`SELECT * FROM locations WHERE id = $1 AND deleted_at IS NULL`, [id]);
}
export async function listLocations(limit = 25, offset = 0, search, channelId, status) {
    ...
    const countResult = await queryOne(`SELECT COUNT(*) as count FROM locations ${whereClause}`, params);
    const items = await queryAll(`SELECT * FROM locations ${whereClause} ORDER BY position_code ASC 
...
export async function updateLocation(id, data) {
    ...
    paramCount++;
    values.push(id);
    await query(`UPDATE locations SET ${updateFields.join(', ')} WHERE id = $${paramCount} AND deleted_at IS NULL`, values);
...
export async function deleteLocation(id) {
    const now = new Date().toISOString();
    await query(`UPDATE locations SET deleted_at = $1 WHERE id = $2`, [now, id]);
}
export async function getLocationsByChannel(channelId) {
    return queryAll(`SELECT * FROM locations WHERE channel_id = $1 AND status = 'ACTIVE' AND deleted_at IS NULL 
     ORDER BY position_code ASC`, [channelId]);
}
/**
 * Check if location can be deactivated
 */
export async function canDeactivateLocation(locationId) {
    const result = await queryOne(`SELECT COUNT(*) as count FROM physical_items 
     WHERE location_id = $1 AND status = 'TREO' AND deleted_at IS NULL`, [locationId]);
 ...
 */
export async function deactivateLocationItems(locationId) {
    const now = new Date().toISOString();
    await query(`UPDATE physical_items SET status = 'INACTIVE', updated_at = $1 
     WHERE location_id = $2 AND status = 'ACTIVE' AND deleted_at IS NULL`, [now, locationId]);
}
